...
class VisionLLMCaptioner:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "model_name": ("STRING", {"default": "gemma-4-E4B.gguf", "multiline": False}),
                "model_path": ("STRING", {"default": "/path/to/Gemma-4-E4B-Uncensored-HauhauCS-Aggressive-Q4_K_M.gguf", "multiline": False}),
                "mmproj_path": ("STRING", {"default": "/path/to/mmproj-Gemma-4-E4B-Uncensored-HauhauCS-Aggressive-f16.gguf", "multiline": False}),
    ...
    def _build_system_prompt(self, raw: str, enable_thinking: bool, image_count: int, image_labels: str) -> str:
        ...
        # Let the handler handle < |think| > injection — do not manually prepend here
        return prompt

    @staticmethod
    def _extract_caption(content: str) -> str:
        """Light cleanup — the new handler already returns clean content."""
        if not content or not content.strip():
            return "[EMPTY RESPONSE]"
        # Only remove obvious control tokens if they somehow leak through
    ...
    def generate(self, backend, mode, server_url, model_name, model_path, mmproj_path,
        ...
        print(f"[DEBUG] START - Backend: {backend} | Mode: {mode} | Thinking: {enable_thinking}")
        ...
        if backend == "Remote API (llama-server)":
        ...
        else:  # === LOCAL STANDALONE (YOUR UPDATED FORK) ===
            if not LLAMA_CPP_AVAILABLE:
                raise ImportError("llama-cpp-python not found. Install your fork.")

            if self.llm is None:
                print(f"[DEBUG] Loading Gemma-4 with updated Gemma4ChatHandler → {model_path}")
                ...
                if HAS_GEMMA4_HANDLER:
                    ...
                    print("[DEBUG] Using updated Gemma4ChatHandler (structured thinking + audio ready)")
                ...
                print("[DEBUG] Gemma-4 loaded successfully")
            ...
            if mode == "Image Caption":
                ...
                if not (input_text or "").strip():
                    raise ValueError("Enter text in 'input_text'.")
                messages = [
                    {"role": "system", "content": self._build_system_prompt(system_prompt, enable_thinking, 0, "(text-only mode)")},
                    {"role": "user", "content": f"{input_text}\n\n{user_prompt}".strip()}
                ]
            ...
            local_params = {
                ...
                "presence_penalty": presence_penalty,
                "seed": seed if seed != 0 else None,
                "enable_thinking": enable_thinking,           # ← NEW: Use the improved handler
            }

            response = self.llm.create_chat_completion(**local_params)
            msg = response.get("choices", [{}])[0].get("message", {})

            # === USE STRUCTURED OUTPUT FROM UPDATED HANDLER ===
            thinking_text = msg.get("thinking", "") if enable_thinking else ""
        ...
        debug_str = (
            f"=== BACKEND: {backend} ===\n"
            f"Mode: {mode} | Thinking: {enable_thinking}\n"
            f"=== THINKING ===\n{thinking_text}\n\n"
            f"=== CAPTION ===\n{caption}\n\n"
            f"=== RAW CONTENT ===\n{answer_text}"
        )
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "VisionLLMCaptioner": "VisionLLMCaptioner - Gemma-4 Vision + Prompt Enhancer (Patched)"
}
